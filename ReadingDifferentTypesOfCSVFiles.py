...
# I need to level with you, I've been lying to you fo rthe past two exercises. Well, kind of. We've been acting like CSV
...
# character in our data is why we pass the newline='' keyword argument ot the open() function.
...
# Notice that when we call csv.DictReader we pass in the delimiter parameter, which is the string that's used to
# delineate separate fields in the CSV. We then iterate through the CSV and print out each of the addresses.
...
with open('books.csv', newline='') as books_csv:
    # 3. Create a DictReader instance that uses the @ symbol as a delimiter to read books.csv. Save the result in a
    ...
    #    file. Use the ['ISBN'] key fo rth edictionary objects passed to it.
    isbn_list = []
